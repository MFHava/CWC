...
BOOST_AUTO_TEST_CASE(ctor) {
	...
	BOOST_TEST(cwc::get<int>(var) == 0);

	var = 10.;
	BOOST_TEST(cwc::holds_alternative<double>(var));
	BOOST_TEST(cwc::get<double>(var) == 10.);

	var = 10;
	BOOST_TEST(cwc::holds_alternative<int>(var));
	BOOST_TEST(cwc::get<int>(var) == 10);
}

BOOST_AUTO_TEST_CASE(copy) {
	cwc::variant<double, int> var{1000};
	BOOST_TEST(!var.valueless_by_exception());
	BOOST_TEST(!cwc::holds_alternative<double>(var));

	auto copy1 = var;
	BOOST_TEST(!copy1.valueless_by_exception());
	BOOST_TEST(!cwc::holds_alternative<double>(copy1));
	BOOST_TEST(cwc::get<int>(var) == cwc::get<int>(copy1));

	decltype(var) copy2; copy2 = copy1;
	BOOST_TEST(!copy2.valueless_by_exception());
	BOOST_TEST(!cwc::holds_alternative<double>(copy2));
	BOOST_TEST(cwc::get<int>(var) == cwc::get<int>(copy2));
}
...
BOOST_AUTO_TEST_CASE(move) {
	cwc::variant<moveable> var1;
	decltype(var1) var2{std::move(var1)};
	BOOST_TEST( cwc::get<moveable>(var1).moved);
	BOOST_TEST(!cwc::get<moveable>(var2).moved);
	var1 = std::move(var2);
	BOOST_TEST(!cwc::get<moveable>(var1).moved);
	BOOST_TEST( cwc::get<moveable>(var2).moved);
}
...
BOOST_AUTO_TEST_CASE(swapping) {
	cwc::variant<int, double> var1{10}, var2{20.2};
	BOOST_TEST(cwc::holds_alternative<int>(var1));
	BOOST_TEST(cwc::holds_alternative<double>(var2));

	swap(var1, var2);
	BOOST_TEST(cwc::holds_alternative<double>(var1));
	BOOST_TEST(cwc::get<double>(var1) == 20.2);
	BOOST_TEST(cwc::holds_alternative<int>(var2));
	BOOST_TEST(cwc::get<int>(var2) == 10);

	decltype(var1) var3{20};
	swap(var2, var3);
	BOOST_TEST(cwc::holds_alternative<int>(var2));
	BOOST_TEST(cwc::get<int>(var2) == 20);
	BOOST_TEST(cwc::holds_alternative<int>(var3));
	BOOST_TEST(cwc::get<int>(var3) == 10);
}
